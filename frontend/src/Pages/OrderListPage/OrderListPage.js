...
export default function OrderListPage() {
  ...
  const changeOrderStatus = async (event, orderId, idx) => {
    try {
      const status = event.target.value
      await axios.put(`/api/orders/${orderId}/change-status-to/${status}`, {
        headers: { Authorization: `Bearer ${userInfo.token}` },
      })
      const newOrders = orders
      newOrders[idx] = { ...orders[idx], status: status, isDelivered: status == 'delivered', isPaid: status == 'delivered' }

      dispatch({ type: 'FETCH_SUCCESS', payload: newOrders });
          ...
          {orders?.map((order, idx) => (
            ...
            >
              <div className="grid grid-cols-7 bg-gray-100 text-sm font-medium text-gray-700 overflow-x-auto">
                ...
                <div
                  className={`py-4 px-4 ${order.isPaid ? 'text-green-600' : 'text-red-600'
                    }`}
                >
                  {order.isPaid ? 'Yes' : 'No'}
                </div>
                <div
                  className={`py-4 px-4 ${order.isDelivered ? 'text-green-600' : 'text-red-600'
                    }`}
                >
                ...
                <div className="py-3 px-auto lg:px-4 text-center flex">
                  ...
                  <select onChange={(e) => changeOrderStatus(e, order._id, idx)} value={order.status}>
